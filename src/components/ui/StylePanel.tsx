...
export const StylePanel: React.FC = () => {
  ...
  return (
    <div
      className={`fixed top-1/2 -translate-y-1/2 left-4 z-40 transition-transform duration-300 ease-out ${
    ...
    >
      ...
      <div className="relative w-72">
        ...
        {/* Attached Vertical Tab Toggle Button (per sidebar.md diagram) */}
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="absolute -right-9 top-1/2 -translate-y-1/2 py-5 px-2 rounded-r-2xl bg-neutral-900 text-white shadow-2xl hover:bg-neutral-800 transition flex flex-col items-center gap-1 font-extrabold text-[10px] tracking-widest uppercase cursor-pointer z-50"
          title={isCollapsed ? 'Открыть панель' : 'Скрыть панель'}
        >
          {isCollapsed ? (
            <>
              <ChevronRight className="w-4 h-4 text-blue-400 mb-1" />
              <span className="[writing-mode:vertical-lr]">ОТКРЫТЬ</span>
            </>
          ) : (
            <>
              <ChevronLeft className="w-4 h-4 text-neutral-400 mb-1" />
              <span className="[writing-mode:vertical-lr]">ЗАКРЫТЬ</span>
            </>
          )}
        </button>

        {/* Inner Content Container with Overflow Scroll */}
        <div className="w-full max-h-[calc(100vh-140px)] overflow-y-auto bg-white/80 dark:bg-neutral-900/80 backdrop-blur-2xl backdrop-saturate-150 border border-white/60 dark:border-neutral-800 shadow-[0_8px_32px_rgba(0,0,0,0.12)] rounded-[24px] p-4 text-xs font-semibold space-y-4">
          ...
          <div className="flex items-center justify-between pb-2.5 border-b border-neutral-200/60 dark:border-neutral-800">
            <div className="flex items-center gap-2">
              ...
              <span className="text-neutral-900 dark:text-neutral-100 uppercase tracking-wider font-extrabold text-[11px]">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-neutral-900 dark:text-neutral-100 font-extrabold">
                Цвет контура
              </label>
              <button
                onClick={() => colorInputRef.current?.click()}
                className="flex items-center gap-1 text-[11px] text-blue-600 hover:text-blue-700 font-bold bg-blue-50/80 px-2 py-0.5 rounded-lg border border-blue-200/60 transition"
              >
          ...
          {!isTextSelected && (
            <div className="space-y-2">
              ...
              {activeStyle.fillColor !== 'transparent' && (
                <div className="grid grid-cols-2 gap-1.5 bg-neutral-100/80 dark:bg-neutral-800/80 p-1 rounded-xl">
                  <button
          ...
          {/* Pro Teaser */}
          <div className="p-2.5 rounded-2xl bg-neutral-100/70 dark:bg-neutral-800/70 border border-neutral-200/60 dark:border-neutral-700 flex items-center justify-between text-[11px] text-neutral-600 dark:text-neutral-400">
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-3 gap-1.5 bg-neutral-100/80 dark:bg-neutral-800/80 p-1.5 rounded-2xl">
              {[
                { label: 'Тонкая', val: 2 },
                { label: 'Средняя', val: 4 },
                { label: 'Толстая', val: 6 },
          ...
          {/* Sloppiness */}
          <div className="space-y-1.5">
            <label className="text-neutral-900 dark:text-neutral-100 font-extrabold flex items-center gap-1.5">
          ...
          {/* Text Sizes */}
          {isTextSelected && (
            <div className="space-y-1.5 border-t border-neutral-200/60 dark:border-neutral-800 pt-3">
              <label className="text-neutral-900 dark:text-neutral-100 font-extrabold">
                Размер текста
              </label>
              <div className="grid grid-cols-4 gap-1.5 bg-neutral-100/80 dark:bg-neutral-800/80 p-1.5 rounded-2xl">
                {[16, 22, 30, 40].map((fs) => (
                  <button
                    key={fs}
          ...
          {hasSelection && (
            <div className="space-y-1.5 border-t border-neutral-200/60 dark:border-neutral-800 pt-3">
              <label className="text-neutral-900 dark:text-neutral-100 font-extrabold flex items-center gap-1">
                <Layers className="w-3.5 h-3.5 text-blue-600" />
                <span>Слои объектов</span>
              </label>
              <div className="grid grid-cols-4 gap-1.5 bg-neutral-100/80 dark:bg-neutral-800/80 p-1.5 rounded-2xl text-neutral-700 dark:text-neutral-300">
                <button
                  onClick={sendToBack}
